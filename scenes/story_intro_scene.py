...
class StoryIntroScene(Scene):
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            ...
            if event.key in (pygame.K_LEFT, pygame.K_BACKSPACE):
                ...
                return
            # Forward: any other key
            if self.timer >= 30:
                self.assets.play_sound("story")
                self._advance()

        if event.type == pygame.MOUSEBUTTONDOWN and self.timer >= 30:
            # Left half of screen = go back, right half = forward
            if event.button == 1:
    ...
    def draw(self, screen):
        ...
        if self.fade_in > 0.5:
            ...
            ds = pygame.Surface((int(box_w * 0.7), 1), pygame.SRCALPHA)
            ds.fill((*col[:3], da))
            screen.blit(ds, (cx - ds.get_width() // 2, title_y + 32))

        # Story lines inside box
        if font_medium:
            ly  = title_y + 48
            gap = int(box_h * 0.085)
        ...
        if self.timer > 30 and font_small:
            ...
            fwd   = "Any key to begin your journey" if last else "Any key / → to continue"
            fwd_s = font_small.render(fwd, True, (100, 100, 120))
            screen.blit(fwd_s, (cx - fwd_s.get_width() // 2, int(sh * 0.93)))

            if self.page_index > 0:
                back_s = font_small.render("← / Backspace to go back", True, (80, 80, 100))
                screen.blit(back_s, (cx - back_s.get_width() // 2, int(sh * 0.96)))